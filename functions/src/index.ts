...
exports.letterboxd = onCall(
  ...
  async (request) => {
    ...
    return getFilm(filmId, token).then((res) => {
      return res.data;
    }).catch((error) => {
      throw new HttpsError(
          "failed-precondition",
...
function getFilmByTmdbId(tmdbId: number, token: string): Promise<AxiosResponse<{items: any[]}>> {
  return axios.get<{items: any[]}>(
      `${baseUrl}films?filmId=tmdb:${tmdbId}`,
      {
...
function getFilm(letterboxId: string, token: string): Promise<AxiosResponse<any>> {
